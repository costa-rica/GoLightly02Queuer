...
import { MantraArrayElement, AudioConcatenatorCsvRow, ChildProcessResult } from '../types';
...
/**
 * Generate AudioConcatenator CSV from mantra data and ElevenLabs output files
...
export function generateAudioConcatenatorCsv(
  ...
  logger.info('Generating AudioConcatenator CSV from mantra data');
  ...
  for (const element of mantraElements) {
    ...
    if (element.text && element.text.trim() !== '') {
      ...
      if (elevenLabsFileIndex < elevenLabsFiles.length) {
        ...
        logger.warn(`No ElevenLabs file found for element ${element.id} (text: "${element.text}")`);
      }
      ...
      logger.warn(`Element ${element.id} has no text, pause_duration, or sound_file - skipping`);
    }
  }
  ...
  logger.info(`Generated AudioConcatenator CSV with ${rows.length} rows: ${csvPath}`);
 ...
 */
export async function spawnAudioConcatenatorProcess(csvPath: string): Promise<ChildProcessResult> {
  ...
  if (!audioConcatenatorPath) {
    throw new Error('PATH_TO_AUDIO_FILE_CONCATENATOR environment variable not set');
  }

  const childAppName = process.env.NAME_CHILD_PROCESS_AUDIO_FILE_CONCATENATOR || 'AudioFileConcatenator01';
  ...
  if (!result.success) {
    ...
    throw new Error(`AudioConcatenator process failed with exit code: ${result.exitCode}`);
  }
...
export function parseAudioConcatenatorOutput(stdout: string): string | null {
  ...
  logger.warn('Could not find final output file path in AudioConcatenator output');
  return null;
}
...
export async function runAudioConcatenatorWorkflow(
  ...
  // Generate CSV
  const csvPath = generateAudioConcatenatorCsv(mantraElements, elevenLabsFiles);
  ...
  if (!finalFilePath) {
    throw new Error('Failed to get final output file path from AudioConcatenator');
  }
