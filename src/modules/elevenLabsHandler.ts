...
import { MantraArrayElement, ElevenLabsCsvRow, ChildProcessResult } from '../types';
...
/**
 * Generate ElevenLabs CSV from parsed mantra data
 * @param mantraElements - Array of mantra elements
 * @returns Object with CSV path and row count
 */
export function generateElevenLabsCsv(mantraElements: MantraArrayElement[]): { csvPath: string; rowCount: number } {
  logger.info('Generating ElevenLabs CSV from mantra data');

  // Filter elements that have text (need to be processed by ElevenLabs)
  const elevenLabsRows: ElevenLabsCsvRow[] = [];

  for (const element of mantraElements) {
  ...
  logger.info(`Generated ElevenLabs CSV with ${elevenLabsRows.length} rows: ${csvPath}`);
 ...
 */
export async function spawnElevenLabsProcess(csvFilename: string): Promise<ChildProcessResult> {
  ...
  const childAppName = process.env.NAME_CHILD_PROCESS_ELEVENLABS || 'RequesterElevenLabs01';
  ...
  if (!result.success) {
    ...
    throw new Error(`ElevenLabs process failed with exit code: ${result.exitCode}`);
  }
 ...
 * @returns Array of generated MP3 file paths
 */
export async function runElevenLabsWorkflow(mantraElements: MantraArrayElement[]): Promise<string[]> {
  logger.info('Starting ElevenLabs workflow');

  // Generate CSV
  const { csvPath, rowCount } = generateElevenLabsCsv(mantraElements);
  ...
  if (filePaths.length !== rowCount) {
    logger.warn(`Expected ${rowCount} files but got ${filePaths.length} from ElevenLabs output`);
  }

  logger.info(`ElevenLabs workflow completed with ${filePaths.length} generated files`);
